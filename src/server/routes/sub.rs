...
/// ```
#[get("/plans")]
pub async fn get_all_plans(config: web::Data<Arc<Config>>) -> impl Responder {
    let client = services::pay::create_client(&config.stripe_secret_key);
...
pub async fn subscribe(
    ...
    let stripe_req = crate::server::dtos::pay::SubscriptionRequest {
...
pub async fn get_my_subscription(
    ...
    let subscription = services::sub::get_user_subscription(&client, &claims.stripe_customer_id)
        .await?
        .ok_or_else(|| {
            crate::server::misc::error::AppError::NotFound(
